...
const url = 'https://playerservices.streamtheworld.com/api/livestream?station=web10&transports=http%2Chls%2Chlsts&version=1.9&request.preventCache=1516699625968';
...
fetch(url)
    ...
    .then(json => {
        ...
        const urls = servers.map(server => `${protocol}://${server.ip}/${mount}`).join('\n');
